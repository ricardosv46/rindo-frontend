  ...
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        'ghost-colored': 'bg-background hover:bg-accent hover:text-accent-foreground',
...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof buttonVariants> {
  ...
  variant?: 'default' | 'destructive' | 'outline' | 'secondary' | 'ghost' | 'ghost-colored' | 'link'
}
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, color = 'primary', size, asChild = false, ...props }, ref) => {
    ...
    // Ghost colored variant (similar to outline but without border)
    if (variant === 'ghost-colored' && color) {
      switch (color) {
        case 'primary':
          customClasses = 'text-primary-600 hover:bg-primary-600/10 hover:text-primary-600'
          break
        case 'green':
          customClasses = 'text-green-600 hover:bg-green-600/10 hover:text-green-600'
          break
        case 'blue':
          customClasses = 'text-blue-600 hover:bg-blue-600/10 hover:text-blue-600'
          break
        case 'yellow':
          customClasses = 'text-yellow-600 hover:bg-yellow-600/10 hover:text-yellow-600'
          break
        case 'red':
          customClasses = 'text-red-600 hover:bg-red-600/10 hover:text-red-600'
          break
        case 'purple':
          customClasses = 'text-purple-600 hover:bg-purple-600/10 hover:text-purple-600'
          break
        case 'gray':
          customClasses = 'text-gray-600 hover:bg-gray-600/10 hover:text-gray-600'
          break
        case 'orange':
          customClasses = 'text-orange-600 hover:bg-orange-600/10 hover:text-orange-600'
          break
      }
    }

    // Determinar si aplicamos el color o no
    const shouldApplyColor = variant !== 'outline' && variant !== 'ghost-colored' && !(size === 'icon' && variant !== 'default')
