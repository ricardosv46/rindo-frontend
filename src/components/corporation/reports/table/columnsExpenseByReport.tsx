...
interface IColumnsExpenseByReport {
  ...
  selection?: boolean
  history?: boolean
  setDataSelectedHistory?: (data: IHistory[]) => void
    ...
    ? [
        {
          ...
          cell: (info: any) => {
            const data = info.row.original
            const handleShowFile = () => {
              const history = data?.history.map((item: IHistory) => ({ ...item, description: data?.description }))
              setDataSelectedHistory?.(history)
              openModalHistory?.()
            }
